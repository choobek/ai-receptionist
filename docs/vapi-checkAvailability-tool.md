...
- broad preference without a date like `po osiemnastej` or `wieczorem`: `timePreference = evening`, optionally without `requestedDate`
- exact hour without a date like `o osiemnastej`: `timePreference = specific_time`, `requestedTime = 18:00`, optionally without `requestedDate`
